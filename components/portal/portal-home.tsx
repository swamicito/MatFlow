...
import { Flame, Target, Award, CreditCard, BookOpen, TrendingUp } from "lucide-react";
...
export function PortalHome({ data }: { data: PortalDashboardData }) {
  ...
  return (
    <div className="space-y-6">
      {/* Greeting */}
      <div>
        <p className="text-xs text-[#555] uppercase tracking-widest">{gym.name}</p>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          {beltProgress && (
            <span className="text-xs text-[#555]">· {beltProgress.stripes} stripe{beltProgress.stripes !== 1 ? "s" : ""}</span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <StatCard icon={Award} label="Badges" value={String(badges.length)} unit="earned" />
      </div>
      ...
      {beltProgress && (
        <section className="rounded-xl border border-[#1a1a1a] bg-[#0a0a0a] p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-[#555]">{beltProgress.progress_percentage}%</span>
          </div>
          <div className="h-2 rounded-full bg-[#111] overflow-hidden">
            <div
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#555] ml-1">{4 - beltProgress.stripes} stripe{4 - beltProgress.stripes !== 1 ? "s" : ""} to next</span>
          </div>
        </section>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="rounded-xl border border-[#1a1a1a] bg-[#0a0a0a] p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-[#555]">Class Credits</p>
        ...
        <div className="rounded-xl border border-[#1a1a1a] bg-[#0a0a0a] p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-[#555]">Videos Owned</p>
      ...
      {activeMembership && (
        <section className="rounded-xl border border-[#1a1a1a] bg-[#0a0a0a] p-4 space-y-1">
          <p className="text-xs text-[#555] uppercase tracking-widest">Active Membership</p>
...
function StatCard({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border bg-[#0a0a0a] p-3 space-y-1",
        highlight ? "border-white/20" : "border-[#1a1a1a]",
      )}
    >
      <div className="flex items-center gap-1.5">
        <Icon className={cn("h-3.5 w-3.5", highlight ? "text-white" : "text-[#555]")} />
      ...
      <p className="text-[10px] text-[#444]">{unit}</p>
    </div>
  );
}
